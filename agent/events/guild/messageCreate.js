...
//         })
    ...
    //  if( message.author.bot) return;
     ...
     // @@@@@@ registrar usuario por mensajes @@@@@@
     let userData;
     try {
        userData = await modelMessage.findOne({userID: message.author.id});
        if(!userData) {
            let user = await modelMessage.create({
    ...
    try {
        ...
        if(!messageData) {
            let mensaje = await modelMessage.create({
